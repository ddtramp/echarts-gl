...
import Grid3DAxis from 'echarts-gl/src/component/grid3D/Grid3DAxis';
...
export default echarts.ComponentView.extend({
  ...
  init(ecModel, api) {
    ...
    if (ecModel.option.onlyOneGrid) {
      showGridConfig = ecModel.option.grid3D[0].showGridConfig || [
        true,
        true,
        true,
      ];
  ...
  render(grid3DModel, ecModel, api) {
    ...
    control.on(
      'update',
      this._onCameraChange.bind(this, grid3DModel, api),
      this,
    );
    ...
    cartesian.viewGL.setTemporalSuperSampling(
      grid3DModel.getModel('temporalSuperSampling'),
    );
  ...
  _updateAxisPointerOnMousePosition(e) {
    ...
    for (let i = 0; i < this._faces.length; i++) {
      ...
      if (
        axis0.contain(point.array[idx0]) &&
        axis1.contain(point.array[idx1])
      ) {
  ...
  _updateAxisLinePosition() {
    ...
    const { yAxisPositionToZero } = this._model.option
    if (yAxisPositionToZero) {

      yAxisNode.position.set(zAxisXOffset, yAxisYOffset, 0); // Actually z
      yAxisNode.rotation.rotateZ(Math.PI);
    } else {
  ...
  _updateAxisPointer(data) {
    ...
    function getAxisColorAndLineWidth(axis) {
      const axisPointerModel = axis.model.getModel(
        'axisPointer',
        axisPointerParentModel,
      );
    ...
    for (let k = 0; k < this._faces.length; k++) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        linesGeo.addLine(
          p0,
          p1,
          colorAndLineWidth.color,
          colorAndLineWidth.lineWidth * dpr,
        );
      }
      ...
      if (ifShowAxisPointer(cartesian.getAxis(faceInfo[2]))) {
        ...
        var colorAndLineWidth = getAxisColorAndLineWidth(
          cartesian.getAxis(faceInfo[2]),
        );
        linesGeo.addLine(
          p0,
          p1,
  ...
  _updateAxisPointerLabelsMesh(data) {
    ...
    this._axes.forEach(function(axisInfo, idx) {
      ...
      const axisPointerModel = axisModel.getModel(
        'axisPointer',
        axisPointerParentModel,
      );
      ...
      if (formatter != null) {
        ...
        const precision = echarts.number.getPrecisionSafe(
          axis.scale.getTicks()[0],
        );
    ...
    axisPointerLabelsMesh.material.set(
      'uvScale',
      axisPointerLabelsSurface.getCoordsScale(),
    );
